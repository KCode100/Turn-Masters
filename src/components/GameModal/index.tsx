    ...
    <>
      <div className="fixed top-0 left-0 right-0 z-50 p-4 min-h-screen max-h-full backdrop-blur-md flex items-center">
        <div className="grow relative bg-white rounded-lg shadow px-6 py-10 text-center w-full max-w-md m-auto">
          {!isFirstRound && (
            <h1 className="score-feature mb-8 text-3xl font-mono text-gray-500 flex items-center justify-center gap-6">
              🎯💰 <span className="text-6xl"><CountUp duration={1} end={points?.newScore || 0} /></span> 💰🎯
            </h1>
          )}
          {!isLastRound && (
            <>
              <div className={!isFirstRound ? `fade-in` : ""}>
                ...
                <h2 className="mb-3 font-mono text-3xl">{nextRoundData.name}</h2>
