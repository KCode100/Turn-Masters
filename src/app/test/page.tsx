...
const TimeGuessGame: FC = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center gap-32">
      <div className={`text-center text-4xl font-mono ${gameComplete ? 'mb-0' : 'mb-12'}`}>
        {isPlaying ? (
          <span>##.##</span>
        ) : (
          <>
        ...
        {gameComplete && (
          <p className={`text-blue-500 block text-2xl text-center pt-4`}>{(SECONDS_TO_GUESS - (time / 1000)).toFixed(2)} seconds out!</p>
        )}
      </div>
